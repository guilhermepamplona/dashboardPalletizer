...
Layout da DB601_DashBoard (TIA Portal, todos os campos Big-Endian):
  ...
  31.2    BOOL   Pick_Conv_1   (pulso: pega todas as caixas visíveis da esteira 1)
  31.3    BOOL   Pick_Conv_2
  31.4    BOOL   Pick_Conv_3
  31.5    BOOL   Pick_Conv_4
  31.6    BOOL   Grip_Release  (pulso: solta todas as caixas no palete)
  31.7    BOOL   Pallet_Conv_1  (pallet no sensor da esteira 1)
  32.0    BOOL   Pallet_Conv_2
  32.1    BOOL   Pallet_Conv_3
  32.2    BOOL   Pallet_Conv_4

  *** Adicione slots de falha conforme fechar a DB no TIA Portal. ***
  *** Atualize NUM_FAULT_SLOTS e DB_SIZE abaixo. ***

Tamanho atual: 33 bytes (cobre offset 32.2)
"""
...
DB_SIZE         = config.DB_SIZE         # 33 bytes (cobre offset 32.2)
POLL_INTERVAL_S = config.POLL_INTERVAL_S

# Número de slots de falha INT na DB.
# Slot 1 → offset 16 (Active_Alarm_Code); slots 2..N → 22, 24, 26, 28, ...
NUM_FAULT_SLOTS = 5
...
class CellState:
    ...
    cycle_time_s: float
    connected: bool
    # Sensores de caixa nas esteiras
    sensor1_conv1: bool
    sensor2_conv1: bool
    sensor1_conv2: bool
    sensor2_conv2: bool
    ...
    sensor2_conv4: bool
    sensor3_conv4: bool
    # Sinais de pick/release (pulso de 1 ciclo vindo do CLP)
    pick_conv1: bool
    pick_conv2: bool
    pick_conv3: bool
    pick_conv4: bool
    ...
    pallet_conv3: bool
    pallet_conv4: bool
    # Alarmes ativos com descrição textual
    ...
    def disconnected() -> "CellState":
        return CellState(
            ...
            cycle_time_s=0.0,
            connected=False,
            sensor1_conv1=False, sensor2_conv1=False,
            ...
            pick_conv1=False, pick_conv2=False,
            pick_conv3=False, pick_conv4=False,
            grip_release=False,
            pallet_conv1=False,
            pallet_conv2=False,
            pallet_conv3=False,
...
def parse_db(raw: bytes) -> CellState:
    ...
    return CellState(
        ...
        pick_conv1=get_bool(raw, 31, 2),
        pick_conv2=get_bool(raw, 31, 3),
        pick_conv3=get_bool(raw, 31, 4),
        pick_conv4=get_bool(raw, 31, 5),
        grip_release=get_bool(raw, 31, 6),
        pallet_conv1=get_bool(raw, 31, 7),
        pallet_conv2=get_bool(raw, 32, 0),
        pallet_conv3=get_bool(raw, 32, 1),
        pallet_conv4=get_bool(raw, 32, 2),
...
class PLCClient:
    ...
    recriando o objeto Client() a cada tentativa para evitar o erro
    'Cannot change this param now'.
    """
